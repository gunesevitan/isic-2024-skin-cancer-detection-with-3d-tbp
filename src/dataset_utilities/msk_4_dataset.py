...
if __name__ == '__main__':
    ...
    dataset_directory = settings.DATA / 'msk-4'
    ...
    df_metadata['id'] = pd.Series(df_metadata.index).apply(lambda x: f'msk-4_{x}')
    df_metadata['dataset'] = 'msk-4'
    ...
    df_metadata.to_parquet(output_directory / 'msk-4-metadata.parquet')
    settings.logger.info(f'msk-4-metadata is saved to {output_directory}')
