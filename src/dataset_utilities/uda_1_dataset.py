...
if __name__ == '__main__':
    ...
    dataset_directory = settings.DATA / 'uda-1'
    ...
    df_metadata['id'] = pd.Series(df_metadata.index).apply(lambda x: f'uda-1_{x}')
    df_metadata['dataset'] = 'uda-1'
    ...
    df_metadata.to_parquet(output_directory / 'uda-1-metadata.parquet')
    settings.logger.info(f'uda-1-metadata.parquet is saved to {output_directory}')
