...
if __name__ == '__main__':
    ...
    dataset_directory = settings.DATA / 'uda-2'
    ...
    df_metadata['id'] = pd.Series(df_metadata.index).apply(lambda x: f'uda-2_{x}')
    df_metadata['dataset'] = 'uda-2'
    ...
    df_metadata.to_parquet(output_directory / 'uda-2-metadata.parquet')
    settings.logger.info(f'uda-2-metadata.parquet is saved to {output_directory}')
