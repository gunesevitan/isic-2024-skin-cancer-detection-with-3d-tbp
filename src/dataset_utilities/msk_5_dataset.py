...
if __name__ == '__main__':
    ...
    dataset_directory = settings.DATA / 'msk-5'
    ...
    df_metadata['id'] = pd.Series(df_metadata.index).apply(lambda x: f'msk-5_{x}')
    df_metadata['dataset'] = 'msk-5'
    ...
    df_metadata.to_parquet(output_directory / 'msk-5-metadata.parquet')
    settings.logger.info(f'msk-5-metadata is saved to {output_directory}')
