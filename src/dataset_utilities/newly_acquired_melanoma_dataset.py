...
if __name__ == '__main__':
    ...
    dataset_directory = settings.DATA / 'newly-acquired-melanoma'
    ...
    df_metadata['id'] = pd.Series(df_metadata.index).apply(lambda x: f'newly-acquired-melanoma_{x}')
    df_metadata['dataset'] = 'newly-acquired-melanoma'
    ...
    df_metadata.to_parquet(output_directory / 'newly-acquired-melanoma-metadata.parquet')
    settings.logger.info(f'newly-acquired-melanoma-metadata is saved to {output_directory}')
