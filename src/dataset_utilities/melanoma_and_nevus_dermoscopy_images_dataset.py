...
if __name__ == '__main__':
    ...
    dataset_directory = settings.DATA / 'melanoma-and-nevus-dermoscopy-images'
    ...
    df_metadata['id'] = pd.Series(df_metadata.index).apply(lambda x: f'melanoma-and-nevus-dermoscopy-images_{x}')
    df_metadata['dataset'] = 'melanoma-and-nevus-dermoscopy-images'
    ...
    df_metadata.to_parquet(output_directory / 'melanoma-and-nevus-dermoscopy-images-metadata.parquet')
    settings.logger.info(f'melanoma-and-nevus-dermoscopy-images-metadata is saved to {output_directory}')
