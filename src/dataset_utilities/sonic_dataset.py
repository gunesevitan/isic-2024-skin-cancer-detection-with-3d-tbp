...
if __name__ == '__main__':
    ...
    dataset_directory = settings.DATA / 'sonic'
    ...
    df_metadata['id'] = pd.Series(df_metadata.index).apply(lambda x: f'sonic_{x}')
    df_metadata['dataset'] = 'sonic'
    ...
    df_metadata.to_parquet(output_directory / 'sonic-metadata.parquet')
    settings.logger.info(f'sonic-metadata is saved to {output_directory}')
