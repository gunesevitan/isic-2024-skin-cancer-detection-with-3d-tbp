...
if __name__ == '__main__':
    ...
    dataset_directory = settings.DATA / 'repeated-dermoscopic-images'
    ...
    df_metadata['id'] = pd.Series(df_metadata.index).apply(lambda x: f'repeated-dermoscopic-images_{x}')
    df_metadata['dataset'] = 'repeated-dermoscopic-images'
    ...
    df_metadata.to_parquet(output_directory / 'repeated-dermoscopic-images-metadata.parquet')
    settings.logger.info(f'repeated-dermoscopic-images-metadata is saved to {output_directory}')
